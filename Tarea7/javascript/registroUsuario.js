document.addEventListener("DOMContentLoaded", function () {
    ...
    registroForm.addEventListener("submit", function (e) {
        ...
        // Validaciones
        // vacios
        if (!username || !nombres || !apellidos || !correo || !celular || !cedula || !vehiculo || !numPlaca || !password || !confirmPassword) {
            showAlert("Por favor, complete todos los campos.", "error");
            return;
        }
        
        if (!/^[a-zA-ZÁÉÍÓÚÜáéíóúü\s]+$/.test(nombres) || !/^[a-zA-ZÁÉÍÓÚÜáéíóúü\s]+$/.test(apellidos) || !/^[a-zA-ZÁÉÍÓÚÜáéíóúü\s]+$/.test(vehiculo)) {
            showAlert("Los campos 'vehiculo', 'nombres' y 'apellidos' solo pueden contener letras, espacios y caracteres acentuados.", "error");
            return;
        }
